...
class AdaptiveServer(ABC):
    def __init__(self, args, config, model, save_interval=50):
        ...
        if self.use_adaptive:
            print("Adjustment dec half-life = {}. "
                  "Adjustment interval = {}.".format(self.config.ADJ_HALF_LIFE,
    ...
    def initial(self, list_est_time, list_loss, list_acc, list_model_size):
        ...
        for server_i in range(1, self.config.IP_MAX_ROUNDS + 1):
            ...
            if (server_i - 1) % self.config.EVAL_DISP_INTERVAL == 0:
                ...
                if ip_start_adj_round is None and train_acc >= self.config.ADJ_THR_ACC:
                    ...
                    print("Start reconfiguration in initial stage at round {}.".format(server_i - 1))
                print("Initial stage round {}. Accuracy = {}. Loss = {}. Train accuracy = {}. Train loss = {}. "
            ...
            for server_inp, server_out in zip(server_inputs, server_outputs):
                ...
                self.control.squared_grad_dict = dict()
            ...
            #     cur_density = disp_num_params(self.model)
                ...
                #     print("Early-stopping initial stage at round {}.".format(server_i - 1))
                #     del list_loss[-1]
                #     del list_acc[-1]
                #     break

        len_pre_rounds = len(list_acc)
        print("End initial stage. Total rounds = {}. Total elapsed time = {}.".format(
            len_pre_rounds * self.config.EVAL_DISP_INTERVAL, timer() - start))

        return len_pre_rounds

    def main(self, idx, list_sd, list_num_proc, lr, list_accumulated_sgrad, list_capacity, start, list_loss, list_acc, list_est_time,
             list_model_size, is_adj_round, density_limit=None):
        total_weight = 0
        for num_proc, capacity in zip(list_num_proc, list_capacity):
            total_weight += num_proc * capacity

        with torch.no_grad():
            for key, param in self.model.state_dict().items():
                avg_inc_val = None
                for num_proc, state_dict, capacity in zip(list_num_proc, list_sd, list_capacity):
                    if key in state_dict.keys():
                        ...
                        if avg_inc_val is None:
                            avg_inc_val = num_proc * capacity / total_weight * inc_val
                        else:
                            avg_inc_val += num_proc * capacity / total_weight * inc_val
        ...
        if self.use_adaptive and is_adj_round:
            # alg_start = timer()
            self.control.squared_grad_dict = dict()
            for num_proc, d in zip(list_num_proc, list_accumulated_sgrad):
                for k, sg in d.items():
                    self.control.accumulate1(k, sg, num_proc)
            self.control.aggregate()
        ...
        for layer, comp_coeff in zip(self.model.prunable_layers, self.config.COMP_COEFFICIENTS):
            est_time += layer.num_weight * (comp_coeff + self.config.COMM_COEFFICIENT) * 0.2
...
class AdaptiveClient:
    def __init__(self, model, config, use_adaptive, capacity, id):
        self.config = config
        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    def main(self, is_adj_round):
            ...
            if self.use_adaptive:
                for key, grad in accumulated_grad.items():
                    if key in self.accumulated_sgrad.keys():
                        self.accumulated_sgrad[key] += (grad ** 2)
                    else:
                        self.accumulated_sgrad[key] = (grad ** 2)
        ...
        if self.use_adaptive and is_adj_round:
            ...
            sgrad_to_upload = {}
        return self.model.state_dict(), num_proc_data, sgrad_to_upload, lr, self.capacity

    def adjust(self,idx):
        max_dec_diff = 1-self.capacity * 1
        self.control.l_adjust(max_dec_diff, max_density=self.capacity)
        # print("Density:",self.capacity)
        # print("Num params:")
        # disp_num_params(self.model)
        return
...
class AdaptiveFL(ABC):
    ...
    def main(self):
        len_pre_rounds = 0
        if self.use_ip:
            print("Starting initial stage...")
            len_pre_rounds = self.server.initial(self.list_est_time, self.list_loss, self.list_acc,
            ...
            for client in self.client_list:
                ...
                client.control.squared_grad_dict = self.server.control.squared_grad_dict
                client.adjust(0)
            print("Starting further pruning stage...")
        
        start = timer()
        for idx in range(self.max_round):
            list_state_dict, list_num, list_accum_sgrad, list_last_lr, list_capacity = [], [], [], [], []
            is_adj_round = False
            if idx % self.config.EVAL_DISP_INTERVAL == 0:
                is_adj_round = self.check_adj_round(len_pre_rounds, idx)

            for client in self.client_list:
                sd, npc, grad, last_lr, cp = client.main(is_adj_round)
                list_state_dict.append(sd)
                list_num.append(npc)
                list_accum_sgrad.append(grad)
                list_last_lr.append(last_lr)
                list_capacity.append(cp)
            ...
            list_mask, new_list_sd = self.server.main(idx, list_state_dict, list_num, last_lr, list_accum_sgrad, list_capacity, start,
