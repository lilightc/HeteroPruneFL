...
def process_layer1(layer, layer_prefix, sgrad: dict, coeff, dtp) -> (float, float, list, torch.Tensor):
    w_name = "{}.weight".format(layer_prefix)
    b_name = "{}.bias".format(layer_prefix)
    sqg = sgrad[w_name]
    iu_mask, niu_mask = layer.mask == 1., layer.mask == 0.
...
def architecture_search1(model, sum_sqg, sum_time, list_coefficient, list_tba_values, list_tba_indices,
                        max_density=None):
    list_len = [len(tba) for tba in list_tba_values]
    list_iter = [iter(tba) for tba in list_tba_values]
    # number of params to be added/removed
...
def main_control1(model, squared_grad_dict: dict, config, dec_thr_pct, max_density=None):
    sum_sqg = 0
    sum_time = config.TIME_CONSTANT
    list_tba_values, list_tba_indices = [], []
    list_coefficient = []
    ...
    for layer, layer_prefix in zip(model.param_layers, model.param_layer_prefixes):
        if layer_prefix in model.prunable_layer_prefixes:
            # coeff = comm_coeff + next(comp_coeff_iter)
            coeff = comm_coeff + 0
            sqg, time, sorted_tba_values, sorted_tba_indices = process_layer1(layer, layer_prefix, squared_grad_dict,
                                                                             coeff, dec_thr_pct)
            sum_sqg += sqg
            sum_time += time
    ...
    print("\tProcessing layers, time = {}.".format(timer() - proc_start))
    nas_start = timer()
    architecture_search1(model, sum_sqg, sum_time, list_coefficient, list_tba_values, list_tba_indices, max_density)
    print("\tNAS time = {}.".format(timer() - nas_start))
...
class ControlModule:
    ...
    def accumulate1(self, key, sgrad, num_proc):
        if key in self.squared_grad_dict.keys():
            self.squared_grad_dict[key] += num_proc * sgrad
            self.proc_num_dict[key] += num_proc
        else:
            self.squared_grad_dict[key] = num_proc * sgrad
            self.proc_num_dict[key] = num_proc

    def aggregate(self):
        for k, sg in self.squared_grad_dict.items():
            self.squared_grad_dict[k] = sg / self.proc_num_dict[k]
    ...
    def l_adjust(self, dec_thr_pct, max_density=None):
        main_control1(self.model, self.squared_grad_dict, self.config, dec_thr_pct, max_density)